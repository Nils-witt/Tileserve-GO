...
func (s *Store) UpsertSyncedMap(ctx context.Context, id uuid.UUID, name string, visibleToAll, anonymousAllowed bool, syncRemoteID uuid.UUID, actor string) (MapRecord, error) {
	...
	// re-applying name/visibility from the remote.
	err := s.db.WithContext(ctx).Exec(`
		...
		SET name = ?, visible_to_all = ?, anonymous_allowed = ?, sync_remote_id = ?, updated_by = ?, updated_at = now()
	`, id, name, visibleToAll, anonymousAllowed, syncRemoteID, actor, actor, actor,
		name, visibleToAll, anonymousAllowed, syncRemoteID, actor).Error
	if err != nil {
...
func (s *Store) HasMapVersion(ctx context.Context, id uuid.UUID, version string) (bool, error) {
	...
	err := s.db.WithContext(ctx).Raw(`SELECT EXISTS(SELECT 1 FROM map_versions WHERE map_uuid = ? AND version = ?)`, id, version).Row().Scan(&exists)
	if err != nil {
...
func (s *Store) RecordSyncedMapVersion(ctx context.Context, id uuid.UUID, version, createdBy string, createdAt time.Time) error {
	...
	`, id, version, createdBy, createdAt).Error
	if err != nil {
...
func (s *Store) SetSyncedCurrentVersion(ctx context.Context, id uuid.UUID, version string) error {
	res := s.db.WithContext(ctx).Exec(`
		UPDATE maps SET current_version = ?, updated_at = now()
		WHERE uuid = ? AND EXISTS (
			SELECT 1 FROM map_versions WHERE map_uuid = ? AND version = ?
		)
	`, version, id, id, version)
	if res.Error != nil {
		return fmt.Errorf("set synced current version: %w", res.Error)
	}
