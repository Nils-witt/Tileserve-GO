...
func userFilterFromQuery(w http.ResponseWriter, r *http.Request) (filter store.UserFilter, ok bool) {
	isAdmin, ok := utils.QueryBoolParam(w, r, "isAdmin")
	if !ok {
		return store.UserFilter{}, false
	}

	canCreate, ok := utils.QueryBoolParam(w, r, "canCreate")
	if !ok {
		return store.UserFilter{}, false
	}

	canEdit, ok := utils.QueryBoolParam(w, r, "canEdit")
	if !ok {
		return store.UserFilter{}, false
	}

	canDelete, ok := utils.QueryBoolParam(w, r, "canDelete")
	if !ok {
		return store.UserFilter{}, false
	}

	canEditGeoObjects, ok := utils.QueryBoolParam(w, r, "canEditGeoObjects")
	if !ok {
		return store.UserFilter{}, false
	}

	canDeleteGeoObjects, ok := utils.QueryBoolParam(w, r, "canDeleteGeoObjects")
	if !ok {
		return store.UserFilter{}, false
	}

	canViewAll, ok := utils.QueryBoolParam(w, r, "canViewAll")
	if !ok {
		return store.UserFilter{}, false
	}
