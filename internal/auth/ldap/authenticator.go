...
// NewAuthenticator validates cfg and returns an Authenticator for it. An
// Authenticator authenticates a username/password pair against an LDAP (or
// Active Directory) directory: it binds a service account to search for the
// user's entry by username, then rebinds on the same connection as that
// entry's DN with the caller-supplied password to actually verify it.
