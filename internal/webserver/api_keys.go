// Package webserver implements the HTTP handlers for tileserve-go's API.
package webserver
...
func APIKeyCreateHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rec, err := st.CreateAPIKey(r.Context(), username, req.Name, auth.UsernameFromContext(r.Context()), req.PublicKeyPEM)
		if err != nil {
...
func APIKeyDeleteHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		id, ok := httputil.PathUUID(w, r, "id", "api key id")
		if !ok {
			return
		}
...
func APIKeyScopesListHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			httputil.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to list api key scopes"})
			return
		}
...
func APIKeyScopeSetHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		mapID, ok := httputil.PathUUID(w, r, "mapId", "map id")
		if !ok {
			return
		}
		...
		httputil.WriteJSON(w, http.StatusOK, scope)
	}
}
