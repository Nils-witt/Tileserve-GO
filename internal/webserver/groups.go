...
func GroupGetHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, ok := httputil.PathUUID(w, r, "id", "group id")
		if !ok {
			return
		}
...
func GroupCreateHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		g, err := st.CreateGroup(r.Context(), req.Name, req.permissions(), req.LDAPGroupDN, req.OIDCGroupClaim, auth.UsernameFromContext(r.Context()))
		if err != nil {
...
func GroupUpdateHandler(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		g, err := st.UpdateGroup(r.Context(), id, req.Name, req.permissions(), req.LDAPGroupDN, req.OIDCGroupClaim, auth.UsernameFromContext(r.Context()))
		if err != nil {
