...
function MapRow({ m, isAdmin, onReload }: { m: MapSummary; isAdmin: boolean; onReload: () => void }) {
  const [error, setError] = useState<string | null>(null);
  ...
  const updateFlag = async (patch: Partial<Pick<MapSummary, "visibleToAll" | "anonymousAllowed">>) => {
    setError(null);
    try {
  ...
  const handleUpload = async (file: File | undefined) => {
    ...
    setProgress(null);
    if (!result.ok) {
      if (!result.unauthorized) setError(result.errorText || `request failed with status ${result.status}`);
    } else {
      onReload();
    }
  };
      ...
      <tr>
        ...
        <td>
          <div className="actions">
            ...
            <button type="button" className="secondary" disabled={uploading} onClick={() => document.getElementById(`upload-${m.uuid}`)?.click()}>
              Upload version
            </button>
            <input
              id={`upload-${m.uuid}`}
            ...
            {progress && (
              <div className="upload-progress">
                <progress value={progress.total ? progress.loaded : undefined} max={progress.total || 100} />
                <span className="upload-progress-label">
                  {progress.total ? `${formatBytes(progress.loaded)} / ${formatBytes(progress.total)}` : "Uploading…"}
                </span>
              </div>
            )}
  ...
  return (
    <div>
      <div className="card">
        ...
        <form className="row" onSubmit={createMap}>
          ...
          <div className="field">
            ...
            <input id="create-version" value={currentVersion} onChange={(e) => setCurrentVersion(e.target.value)} />
          </div>
          <div className="field">
            <label>
              <input type="checkbox" checked={visibleToAll} onChange={(e) => setVisibleToAll(e.target.checked)} /> Visible to all
            </label>
          </div>
          <div className="field">
            <label>
              <input type="checkbox" checked={anonymousAllowed} onChange={(e) => setAnonymousAllowed(e.target.checked)} /> Anonymous
              tile access
            </label>
          </div>
      ...
      <div className="card">
        ...
        <p className="muted">
          Maps are private by default — only admins, the owner, and users granted access under "Permissions" can see them, unless
          "Public" is checked. "Anonymous" separately controls whether tile files (<code>/maps/&lt;uuid&gt;/version/&lt;version&gt;/...</code>)
          can be fetched without signing in at all, regardless of "Public".
        </p>
      </div>
    </div>
  );
